...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function createSuggestionItem(point) {
   ...
   /**
   * Accept a suggestion - improved version with proper sentence handling
   */
  function acceptSuggestion(id) {
    ...
    try {
      // Get all friction points that belong to the same sentence
      const sameSentencePoints = frictionPoints.filter(p => p.sentence === point.sentence);
      const sameSentenceIds = sameSentencePoints.map(p => p.id);
      ...
      if (point.replacement && point.replacement.trim()) {
        ...
        if (!success && point.translated_sentence) {
          // Fallback: replace entire sentence
          console.warn(`⚠️ Single word replace failed, using sentence replacement`);
        ...
        // Just remove the marking
        removeMarkingOnly(point);
      }
      
      // Remove only this specific point from tracking
